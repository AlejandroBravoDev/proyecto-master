...
export default function CriticalInventoryWidget({ ingredients = [] }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 flex flex-col justify-between min-h-[280px]">
      <h2 className="text-lg font-bold text-[#584235] mb-4">Estado de inventario (Crítico)</h2>
      ...
      {!hasIngredients ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
          ...
          <p className="text-sm font-semibold text-[#584235]">Inventario en estado óptimo</p>
            ...
            return (
              <div key={item.id || idx} className="space-y-1.5">
                <div className="flex justify-between items-center text-xs font-bold">
                  <span className="text-[#584235] truncate max-w-[70%]">{item.name}</span>
                  <span className={isCritical ? 'text-[#E63946] font-extrabold' : 'text-slate-400'}>
                    {percentage}% Restante
                  </span>
                </div>
                ...
                <div className="w-full h-3 bg-slate-200/70 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      isCritical ? 'bg-[#E63946]' : 'bg-[#FF7A00]'
                    }`}
